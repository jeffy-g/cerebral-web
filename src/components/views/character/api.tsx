...
const bindESIHealthStatus = async () => {
    ...
    const hsfinder = (health: ESIHealthStatus) => {
        return health.route === endpoint && health.method === method;
    };
    ...
    for (const opnode of opidNodes) {
        ...
        if (re) {
            endpoint = re.context.endpoint;
            method = opid.substring(0, opid.indexOf("_"));
            const healthState = esiHealths.find(hsfinder);
